...
from server.modules import KeyValue
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"key": "my_key", "value": "my_value", "detail": "some info"},
            200,
            "application/json",
        ),
...
def test_set_endpoint(data: Any, status_code: int, content_type: str) -> None:
    response = client.post(
        "/set", json=data, headers={"content-type": content_type}
    )
...
def test_get_endpoint(data: Any, key: str, status_code: int) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"dividend": 10, "divider": 2.5, "detail": "some info"},
            200,
            "application/json",
        ),
        ("string", 415, "text/plain"),
...
def test_divide_endpoint(
    data: Any, status_code: int, content_type: str
) -> None:
    response = client.post(
        "/divide", json=data, headers={"content-type": content_type}
    )
    assert response.status_code == status_code
    if response.status_code == status.HTTP_200_OK:
        assert (
...
def test_not_allowed_path(path: str, req_type: str) -> None:
    match req_type:
        case "GET":
            assert (
        ...
        case "PUT":
            assert (
                client.put(path).status_code
                == status.HTTP_405_METHOD_NOT_ALLOWED
            )
        case "DELETE":
            assert (
                client.delete(path).status_code
                == status.HTTP_405_METHOD_NOT_ALLOWED
            )
        case "POST":
            assert (
        ...
        case "PATCH":
            assert (
                client.patch(path).status_code
                == status.HTTP_405_METHOD_NOT_ALLOWED
            )
        case "OPTIONS":
            assert (
                client.options(path).status_code
                == status.HTTP_405_METHOD_NOT_ALLOWED
            )
        case "HEAD":
            assert (
                client.head(path).status_code
                == status.HTTP_405_METHOD_NOT_ALLOWED
            )
